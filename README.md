...
.
├── Config/
│   └── api_keys.env                 # DEEPSEEK_API_KEY and OPENAI_API_KEY (gitignored)
├── Prompts/
│   ├── lexical_prompt.txt
│   ├── persona1_prompt.txt
│   ├── persona2_prompt.txt
...
9.1.parse_llm_judge_evaluation.py — extracts all the sub-dimensions of voice preservation, meaning preservation, and accessibility.
...
check_api.py — quick OpenAI connectivity / rate-limit check. Loads the key from the env file.
